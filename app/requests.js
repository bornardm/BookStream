...
export function fetchBookInfos({ id }) {
    ...
    try {
        dbConnexion.withTransactionSync(() => {
            ...
            result = dbConnexion.getFirstSync(
 ...
 */
function getExecSyncDB({ request, params = [], connexion=dbConnexion }) {
    //console.log("DB : start fetching execSync");
    let result = null;
    try {
        connexion.withTransactionSync(() => {
            //console.log("transaction start ");
            //console.log("request:", request, " ; params:", params);
            result = connexion.getAllSync(request, params);
            //console.log("Rows:", result);
            //console.log("transaction end");
        });
        return result;
...
//Functions to check if the database as the expected structure
/**
 * Checks if the database has the expected structure.
 * 
 * @param {Object} connexion - The database connection object.
 * @returns {boolean} - Returns true if the database structure is valid, false otherwise.
 */
export function checkDbStructure(connexionToDB) {
    //console.log("DB : start checking DB structure");
    const requestBooks = `SELECT id, title, author, publicationDate, publisher, pageNumber, isbn, summary, status, borrowed, toExchange, rating, readingStartDate, readingEndDate, imageName, series, volume, comment, categories language, addedDate FROM BOOKS LIMIT 1 ; `;
    const requestSettings = `SELECT id, language, sort, defaultStatus, dbVersion FROM SETTINGS LIMIT 1`;
    console.log("dbConnexion", dbConnexion);
    const result = getExecSyncDB({ request: requestBooks + requestSettings, connexion: connexionToDB});
    if (result) {
        console.log("DB : structure is OK");
        return true;
    }
    console.log("DB : structure is NOT OK");
    return false;
